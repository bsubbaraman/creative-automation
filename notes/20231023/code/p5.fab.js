...
class Fab {
  constructor(config = defaultPrinterSettings) {
    ...
    this.v = 0;
    this.e = 0;
    this.lastX = 0; // adding these in to try subdividing commands
    this.lastY = 0;
    this.lastZ = 0
    this.speed = 0;
    this.nozzleT = 0;
  ...
  add(cmd) {
    // let cmds = this.subdivideMoveCommands(cmd); // adding in for midi control
    // cmds.forEach((c) => this.commands.push(c));
    this.commands.push(cmd);
  }

  subdivideMoveCommands(cmd) {
    // this doesn't work yet!
    // TODO: should first remove any comments
    // and accommodate G3
    let subdivide = [];
    let moveCommands = ['G0', 'G1', 'G2', 'G3']; // currently only for linear movements, i.e. G3 doesn't work
    let splitCmd = cmd.split(' ');
    let code = splitCmd[0];
    if (moveCommands.indexOf(code) > -1) { // if it's a move command
      console.log("currently subdividing:", cmd)
      let subdivide = [];
      console.log(`moving from (${this.lastX}, ${this.lastY}, ${this.lastZ}) to (${this.x}, ${this.y}, ${this.z})`)
      let moveLength = dist(this.lastX, this.lastY, this.lastZ, this.x, this.y, this.z);
      if (!cmd.includes('X') && !cmd.includes('Y') && !cmd.includes('Z')) {// omit retracts/primers
        console.log('no x/y/z, returning')
        return [cmd];
      }

      console.log("move length is:", moveLength);

      if (moveLength > 1) { // divide moves into ~1mm sections
        console.log('subdividing')
        let slope3D = new p5.Vector(this.x - this.lastX, this.y - this.lastY, this.z - this.lastZ).div(moveLength);
        for (let t = 1; t <= moveLength; t += 1) {
          let subX = this.lastX + t * slope3D.x;
          console.log(subX);
          let subY = this.lastY + t * slope3D.y;
          let subZ = this.lastZ + t * slope3D.z;
          subX = parseFloat(subX).toFixed(2);
          subY = parseFloat(subY).toFixed(2);
          subZ = parseFloat(subZ).toFixed(2);
          subdivide.push(`${code} X${subX} Y${subY} Z${subZ} E0.05`) // TODO: F field & calculate E
        }
      }
      else {
        return [cmd]
  ...
  updateWithMidiValues(cmd) {
    // for now, just try modifying speed with top left knob (id 16)
    // and extrusion with the one next to it (id 20)
    // TODO: should first remove any comments
    let moveCommands = ['G0', 'G1', 'G2', 'G3'];
    console.log('midi value from fab:', _midiController.speed);
    let splitCmd = cmd.split(' ');
    let code = splitCmd[0];


    if (moveCommands.indexOf(code) > -1) { // if is move command
      if (_midiController.speed) {
        if (cmd.includes('F')) { // if F field already there, change it
          var searchString = 'F';
          var fIndex = splitCmd.findIndex((findF) => { return findF.startsWith(searchString); }, searchString)
          splitCmd[fIndex] = ` F${_midiController.speed}`
          cmd = splitCmd.join(' ')
        }
        else { // add F directly
          cmd += `F${_midiController.speed}`
        }
      }

      if (_midiController.extrusionMultiplier) {
        splitCmd = cmd.split(' ');
        if (cmd.includes('E')) {
          var searchString = 'E';
          var eIndex = splitCmd.findIndex((findE) => { return findE.startsWith(searchString); }, searchString)
          let eValue = splitCmd[eIndex].substring(1);
          eValue = parseFloat(eValue) * _midiController.extrusionMultiplier;
          splitCmd[eIndex] = ` E${eValue}`
          cmd = splitCmd.join(' ')
        }

        // have up multiplier mean slower speed
        if (cmd.includes('F')) { // if F field already there, change it
          splitCmd = cmd.split(' ');
          var searchString = 'F';
          var fIndex = splitCmd.findIndex((findF) => { return findF.startsWith(searchString); }, searchString)
          let mappedSpeed = map(_midiController.extrusionMultiplier, 0.5, 5, 2000, 50).toFixed(2);
          splitCmd[fIndex] = ` F${mappedSpeed}`
          cmd = splitCmd.join(' ')
        }

        // and pop it up in z a little bit too
        if (cmd.includes('Z')) {
          splitCmd = cmd.split(' ');
          var searchString = 'Z';
          var zIndex = splitCmd.findIndex((findZ) => { return findZ.startsWith(searchString); }, searchString)
          let zValue = parseFloat(splitCmd[zIndex].substring(1));
          console.log('old z', zValue);
          let newZ = zValue + parseFloat(map(_midiController.extrusionMultiplier, 0.5, 5, 0, 2));
          newZ = newZ.toFixed(2)
          console.log('new z', newZ);
          splitCmd[zIndex] = ` Z${newZ}`
          cmd = splitCmd.join(' ')
        }
      }
    }
  ...
  serial_ok(g) {
    ...
    g.interactivePrintStream(); // test interactive print
  }
