...
class Fab {
    ...
    add(cmd) {
        // testing interactive midi control
        // if the command in front of this move only 1 axis, and this command moves another,
        // combine them
        // let currentCommandSplit = cmd.split(' ');
        // let currentAxis = '';

        // let prevAxis = ''
        // if (currentCommandSplit[0] == "G0") {
        //     currentAxis = this.checkCommandAxis(cmd)
        // }

        // if (this.commands.length > 0) {
        //     let prevCommand = this.commands.at(-1)
        //     let prevCommandSplit = prevCommand.split(' ')
        //     if (prevCommandSplit[0] == 'G0') {
        //         prevAxis = this.checkCommandAxis(prevCommand)
        //     }


        //     if (currentAxis && prevAxis) { // i.e. we have 2 consecutive G0 commands
        //         // find out if these commands are moving different axes
        //         // checkCommandAxis returns [x, y, z] true/false values
        //         if (currentAxis.filter(Boolean).length == 1 && prevAxis.filter(Boolean).length == 1) { // for now, only consolidate single axis movements
        //             const prevIndices = prevAxis.reduce(
        //                 (out, bool, index) => bool ? out.concat(index) : out,
        //                 []
        //             );
        //             const currentIndices = currentAxis.reduce(
        //                 (out, bool, index) => bool ? out.concat(index) : out,
        //                 []
        //             )
        //             if (prevIndices !== currentIndices) {
        //                 let newCommand = `G0 ${currentCommandSplit[1]} ${prevCommandSplit[1]}`
        //                 this.commands.pop(); // remove last val
        //                 this.commands.push(newCommand);
        //                 console.log('I consolidated to:', newCommand)
        //                 return
        //             }
        //         }
        //     }
    ...
    checkCommandAxis(cmd) {
        let x, y, z;
        x = cmd.includes('X') ? true : false;
        y = cmd.includes('Y') ? true : false;
        z = cmd.includes('Z') ? true : false;
